...
class _AccelerometerScreenState extends State<AccelerometerScreen> {
  ...
  List<List<dynamic>> _data = []; // List to store accelerometer data
  ...
  void _showSuccessDialog(String path) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Accelerometer data saved as CSV file: $path'),
        actions: [
          TextButton(
            onPressed: () {
  ...
  Future<void> _shareFile(String filename) async {
    ...
    final shareResult = await Share.shareXFiles(
      [file],
      text: 'Accelerometer data CSV file',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Accelerometer Screen')),
      body: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: <Widget>[
            SensorDataCard(label: 'X', value: _x),
            SensorDataCard(label: 'Y', value: _y),
            SensorDataCard(label: 'Z', value: _z),
            SizedBox(height: 20),
